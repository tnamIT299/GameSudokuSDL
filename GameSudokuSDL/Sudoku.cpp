...
Sudoku::Sudoku::Sudoku()
	: mWindowHeight(600), mWindowWidth(900),
	mGridHeight(600), mGridWidth(900),
...
void Sudoku::Sudoku::preloadTextures()
{
	...
	loadTexture(mTextureCache[15], "Quit", fontColour);
}
...
void Sudoku::Sudoku::createInterfaceLayout()
{
	...
	mQuitButton.setTexture(mTextureCache[15]);
	Button* otherButtons[numberOfOtherButtons] = { &mCheckButton, &mNewButton, &mSolveButton, &mQuitButton };

	// Redefine button width
	// mGridWidth = 3 * thickBorder + 9 * numberOfOtherButtons (rearrange this equation)
	buttonWidth = (mGridWidth - 5 * thickBorder) / numberOfOtherButtons;
...
void Sudoku::Sudoku::play()
{
	...
	while (!stop)
	{
		...
		while (SDL_PollEvent(&event) != 0)
		{
			...
			//Hadndle  mouse event for "Solve" button
			if (mSolveButton.getMouseEvent(&event) == ButtonState::BUTTON_MOUSE_DOWN) {

			}

			//Hadndle  mouse event for "Quit" button
			if (mQuitButton.getMouseEvent(&event) == ButtonState::BUTTON_MOUSE_DOWN) {
				checkquit = true;
		...
		// Render Quit button
		mQuitButton.renderButton(mRenderer);
		mQuitButton.centerTextureRect();
		mQuitButton.renderTexture(mRenderer);
		...
		// Load and render timer (TO DO: use preloaded textures to render timer) , v? thanh th?i gian trên b?ng 
